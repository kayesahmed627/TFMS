...
@Component({
  selector: 'app-staff-detail-edit',
  templateUrl: './staff-detail-edit.component.html',
  styleUrl: './staff-detail-edit.component.css'
})
export class StaffDetailEditComponent implements OnInit {
  ...
  addAttendence(att?:Attendence){
    if(att){
      this.attendences.push(new FormGroup({
        attendenceDate: new FormControl(att.attendenceDate, Validators.required),
        startTime: new FormControl(att.startTime, Validators.required),
        endTime: new FormControl(att.endTime, Validators.required),
      }));
    }
    else
    {
  ...
  removeAttendence(index:number){
   ...
   save(){
    ...
    this.attendenceService.updateAttendence(this.staffDetail)
    .subscribe({
      next:r=>{
  ...
  ngOnInit(): void {
    ...
    this.attendenceService.getByIdInclude(id)
    .subscribe({
      next: r=>{
        this.staffDetail=r;
        this.staffForm.patchValue(this.staffDetail);
        this.staffDetail.attendences?.forEach(x => {
          this.addAttendence(x);
        });
      },
      error:err=>{
        console.log(err.message || err);
